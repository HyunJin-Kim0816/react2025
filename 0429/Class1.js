...
function Class1(props) {
  ...
  return (
    <>
      ...
      <p>리엑트에서 UI를 표현하고자 할 때 쓰는 문법</p>
      <p>자바스크립트의 확장문법 ,XML과 매우 비습합니다.</p>
      <p>JSX를 반드시 사용해야 하는 것은 아니지만 JSX를 사용하면
        REACT애플리케이션을 더 쉽게 작성이 가능하다.
      </p>
      ...
      <ul>
        ...
        <li>닫는 태그는 반드시 작성해야한다. br / 처럼 태그는 무조건
          닫혀있어야 하며, 종료 태그가 없는 태그는 끝에/를 넣어준다.
        </li>
        <li>JSX의 표현식 {} : 콤포넌트 UI를 구현하는 것 이외에도
          여러가지 기능을 합니다. 그 중 하나가 자바스크립트 값을 처리하는 것입니다.{}
          내부에는 변수값, 속성값 또는 기타 유효한 JAVASCRIPT표현식을 작성할 수 있습니다.
        </li>
        <li>JSX {} 안의 주석은 /**/만 사용가능</li>
        <li>조건부 렌더링 -삼향조건연산자 문법 =〉 (조건식)?(참일때 표현식):(거짓일 경우 표현식)</li>
        <li>JSX문법에서 CSS스타일 작성하는 5가지 방법</li>
        <ol className='border1'>
          ...
          <li id="bg_color1">id 선택자로 서식 적용하기</li>
          <li>css모듈방식</li>
        </ol>
        <li>부모와 자손 콤포넌트(class, function)</li>
        <li>리엑트에서는 파일명을 저장시 앞글자는 대문자로 저장을 한다.(App.js)</li>
      </ul>
    </>
  );
}
